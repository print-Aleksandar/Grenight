...
from agents.res_net_dueling_double_dqn_self_play.network import Network
...
class Agent:
    ...
    def __init__(self, num_planes, rows, columns, num_actions, device="cpu",
                 lr=1e-4, gamma=0.995, buffer_capacity=100_000,
    ...
    def calculate_td_loss(self, state, legal_mask, action, reward,
        ...
        with torch.no_grad():
            ...
            if non_terminal.any():
                ...
                next_q = next_target.gather(1, next_action.unsqueeze(1)).squeeze(1)
